...
class Model(nn.Module):
    def __init__(self):
        ...
        ck1 = torch.load('checkpoint/encoder_Q.ckpt', map_location=device, weights_only=False)
        ...
        ck2 = torch.load('checkpoint/decoder_Q.ckpt', map_location=device, weights_only=False)
...
def init_model(mod):
    for key, param in mod.named_parameters():
        ...
        if param.requires_grad:
            param.data = c.init_scale * torch.randn_like(param.data)
            if split[-2] == 'conv5':
                param.data.fill_(0.)
